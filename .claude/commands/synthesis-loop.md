...
You are the CURATOR (see CLAUDE.md / AGENTS.md). Run the SYNTHESIS loop per
[`tools/SYNTHESIS.md`](../../tools/SYNTHESIS.md) — read it now.

**Precondition**: **no ingest loop is running on this repo right now** (never both loops
at once — two agents editing one working tree corrupts it).
...
3. Promote each genuinely-**new** finding (framework, bio fact, position change,
   ...
   contradictions flagged with a `> ⚠️ CONTRADICTION:` callout. Drop pure repeats (they
   stay L2). Quality over volume: 5 real promotions beat 20 bloated pages.
