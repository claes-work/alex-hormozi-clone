...
Usage:
  python tools/merge_staging.py --channel @AlexHormozi \
      --videos pipeline/staging-alexhormozi-videos.csv \
      --shorts pipeline/staging-alexhormozi-shorts.csv
"""
import argparse
import csv
import re
...
# generic "competition"/"vs"/"[Ep NN]" are deliberately excluded because his SOLO
# videos use them ("DESTROY Your Competition", his solo podcast episodes).
P3_RE = re.compile(
    r"(\bw/\b|\bft\.?\b|\bfeat\b|interview|podcast|\breacts?\b|"
    r"entrepreneurs compete|ask alex|asks alex|q&a|q & a|"
    r"answering your|\bama\b|sits? down|\bguest\b)",
    re.IGNORECASE,
)
# Landmark solo markers -> P1 (book launch / origin / keynote).
P1_RE = re.compile(
    r"(\$100m|100m offers|100m leads|money models|how i (built|made|went|scaled|"
    r"started|lost)|my story|origin story|keynote|gym launch|my first million|"
    r"how i got|how i turned|net worth|billion dollar|from \$?\d+.*to \$?\d+|"
    r"broke to|zero to)",
    re.IGNORECASE,
)
...
def main() -> None:
    ...
    ap.add_argument("--channel", required=True, help="Handle to stamp, e.g. @AlexHormozi")
