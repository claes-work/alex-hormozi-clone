...
USAGE
    ...
    python tools/ingest_batch.py prepare --channel @MoreMozi --n 10

    # Preview the selection only, no network / no changes:
    python tools/ingest_batch.py prepare --channel @MoreMozi --n 10 --dry-run
...
def channel_slug(handle: str) -> str:
    """@MoreMozi -> moremozi ; matches the raw/youtube/<slug>/ dirs and fetch_captions.ps1."""
    return handle.lstrip("@").lower()


def normalize_channel(arg: str) -> str:
    """Accept '@MoreMozi', 'moremozi', 'MoreMozi' -> canonical '@MoreMozi' from the ledger."""
...
Fidelity: cite; verbatim vs paraphrase; DATE opinions; attribute per speaker (only Alex trains
...
def main() -> None:
    ...
    pp.add_argument("--channel", required=True, help="e.g. @MoreMozi (or moremozi)")
